...
When Mandelbrot measured the length of the coastline as multiples of the ruler length $x$ (i.e., $N(x)x$, where $N(x)$ is the number of rulers needed given the length $x$), the function $f(x)$ diverged to reach infinity as $x \rightarrow 0$, because he found finer, but similar zig-zag patterns over and over as he uses a shorter ruler (see the figure below). This observation is realized as a power-law scaling of length $y$:
...
[^1]: Formally, a function $f(x)$ is said to be scale invariant if it suffices the following equation: $f(\lambda x) = \lambda^z f(x)$, where $\lambda$ is an arbitrary constant and $z$ is the scaling exponent. A power law is an example of satisfying this equation.. A power law is one example of satisfying this equation.
